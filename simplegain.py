...
def beg() -> None:
    ...
    if random.random() <= 0.2:
        ...
        print(
            '"'
            + utils.choice("yesbeg").format(utils.money(given_money, colours.OKBLUE))
            + '"'
        )
    else:
        # No money :(
    ...
    if items_got:
        ...
        for item, count in item_count.items():
            ...
            items_received.append(
                f"{articles[count - 1]} {colours.OKBLUE}{item}{colours.ENDC}"
            )
        message_parts += items_received
    if lost_items:
        if lost_items == 1:
            message_parts.append(
                f"{colours.WARNING}an item that wasn't picked up because your inventory was full{colours.ENDC}"
            )
        else:
            message_parts.append(
                f"{colours.WARNING}some items that weren't picked up because your inventory was full{colours.ENDC}"
            )
    if not message_parts:
        message_parts.append(f"{colours.BOLD}NOTHING!{colours.ENDC}")

    if len(message_parts) == 1:
        print(f"You found {message_parts[0]}")
    else:
        print(
            "You found",
